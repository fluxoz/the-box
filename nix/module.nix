...
in
{
  options.services.the-box = {
      ...
      type = lib.types.attrsOf (lib.types.submodule {
        options = {
          ...
          gpu = lib.mkOption {
            ...
            default = false;
            description = ''
              Hand this container every GPU the Box has. Needs the Box's gpu
              axis set (channel gpu = "nvidia" or "amd"), which installs the
              driver layer and decides what "every GPU" means here; without
              it this is ignored.
            '';
          };
        };
    ...
    gpuContainerDevices = lib.mkOption {
      type = lib.types.listOf lib.types.str;
      default = [ "nvidia.com/gpu=all" ];
      description = ''
        The device strings a gpu = true container receives. The default is
        the NVIDIA CDI name (generated by the gpu = "nvidia" axis); the
        gpu = "amd" axis overrides this to the /dev/dri render nodes, which
        is all Vulkan compute needs.
      '';
    };
